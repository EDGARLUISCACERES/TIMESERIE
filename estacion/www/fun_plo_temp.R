fun_plo_temp <- function(bd, f1, f2){
  
  bd0 <- 
    bd %>% 
    dplyr::mutate(
      date = date+lubridate::hours(5),
      Validado = ifelse(TEMP_estado == "Validado", TEMP_celc, NA_real_),
      Prevalidado = ifelse(TEMP_estado == "Prevalidado", TEMP_celc, NA_real_)) %>% 
  ...
  tituloprincipal <- iconv("Evoluci\u00F3n horaria de la temperatura (\u2103)", from = "UTF-8", to = "UTF-8")
  tituloejey <- iconv("Temperatura (\u2103)", from = "UTF-8", to = "UTF-8")
  tituloseriey <- iconv("Temp validado (\u2103)", from = "UTF-8", to = "UTF-8")
  tituloseriey2 <- iconv("Temp prevalidado (\u2103)", from = "UTF-8", to = "UTF-8")
